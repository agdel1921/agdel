...
def print_recommendations(W1, Q1, Q_hat3, movie_titles, type1 = "original"):
        ...
        #    Q_hat[Q_hat < 1] *= 5
            ...
            for jj, movie_id_top in zip(range(m), movie_ids):
                ...
                og_recommend[jj+1][1]= np.array(', '.join(str([movie_titles for ii, qq in enumerate(Q1[jj])if qq > 1])))
                # qq > 1 represents, more than 1 product purchased
                print ('User {} liked {}'.format(jj + 1, ', '.join(([movie_titles[ii]for ii, qq in enumerate(Q1[jj])if qq > 2]))))
                print('\n')
                og_recommend[jj+1][2]= np.array(", ".join([movie_titles[ii] for ii, qq in enumerate(Q1[jj]) if qq < 2 and qq != 0]))
                print ('User {} kinda liked {}\n'.format(jj + 1, ', '.join([movie_titles[ii] for ii, qq in enumerate(Q1[jj]) if qq < 2 and qq != 0])))
