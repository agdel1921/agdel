...
path1_h1b1= "C:/data/170810/vd_test/test_1/"
pathOp_h1b1 = 'C:/data/170810/vd_test/completed_1/'
pathTrained_h1b1 = 'C:/data/170810/vd_test/trained_1/'
...
def print_recommendations(W1, Q1, Q_hat3, movie_titles, type1 = "original"):
        #    Q_hat -= np.min(Q_hat)
        #    Q_hat[Q_hat < 1] *= 5
            og_recommend = np.zeros(shape = (Q1.shape[0]+1,35), dtype= np.ndarray)
            og_recommend[0] = np.array(['User Id', 'Really liked', 'Kinda liked', 'Count of total current products (BP+CMP)','Most Common BP','Recommended Product 1', 'Propensity of Recommendation 1', 'Recommended Product 2', 'Propensity of Recommendation 2', 'Recommended Product 3', 'Propensity of Recommendation 3', 'Recommended Product 4', 'Propensity of Recommendation 4', 'Recommended Product 5', 'Propensity of Recommendation 5',  'Recommended Product 6', 'Propensity of Recommendation 6', 'Recommended Product 7', 'Propensity of Recommendation 7', 'Recommended Product 8', 'Propensity of Recommendation 8', 'Recommended Product 9', 'Propensity of Recommendation 9', 'Recommended Product 10', 'Propensity of Recommendation 10' ,'Recommended Product 11', 'Propensity of Recommendation 11', 'Recommended Product 12', 'Propensity of Recommendation 12', 'Recommended Product 13', 'Propensity of Recommendation 13', 'Recommended Product 14', 'Propensity of Recommendation 14', 'Recommended Product 15', 'Propensity of Recommendation 15' ])    
            Q_hat3 -= np.min(Q_hat3)
            if type1!="original":
                Q_hat3 *= float(1) / np.max(Q_hat3)
                tmp1 = Q_hat3 - 1 * W1
                movie_ids = np.argsort(-tmp1)[:,:15]
...
for x in (X):
    #print x
    if x[-4:] ==".csv":
        if "pd_recommended"+x in os.listdir(pathOp_h1b1) or "pd_recommended"+x+".csv" in os.listdir(pathOp_h1b1):
            continue
        print x
        data = None
        # --- Read Data --- #
