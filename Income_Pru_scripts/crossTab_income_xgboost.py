...
amgPd = pd.read_csv(pathread+'final_data_temp_xboost.csv', low_memory = True)
amgPd.columns
print (amgPd.shape)
amgPd.head()
...
amgPd.columns = ['hid', 'customerseqid', 'customerstatus', 'genderph', 'policyseqid',
       ...
       'productsubcategory', 'ismain', 'Category_list', 'Category_data',
       'householdid', 'premiumbucket', 'PremiumCategory', 'EducationLevel',
...
len(amgPd_w_time)#210205
#amgPd_w_time.to_csv(pathwrite+'tenureData.csv', header = True, index = False)
# training data till 31/03/2016  # records

amgPd_w_time1 = amgPd_w_time[amgPd_w_time['PolicyStartDate'] <= '2016-03-31 00:00:00.0000000']

##REmoving Health
amgPd_w_time1 = amgPd_w_time[amgPd_w_time['Category_list'] != 2]
#amgPd_w_time1['Category_list'] = amgPd_w_time1['Category_list'].replace('Savings ','Savings')
amgPd_w_time.to_csv(pathwrite+'xgboost_Training_Data.csv', header = True, index = False)
...
amgPd_w_time2.to_csv(pathwrite+'xgboost_Testing_Data.csv', header = True, index = False)
