...
temp = np.random.rand(len(amgPd)) < 0.67
...
np.savetxt(pathwrite+'training_recom1_method1.csv', dfMtrx, delimiter=",",fmt='%s')

### 1.1.2 hid Vs Category_list test data
user_u = list(sorted(amgPd_test.hid.unique()))
item_u = list(sorted(amgPd_test.Category_list.unique()))

row = amgPd_test.hid.astype('category', categories=user_u).cat.codes
col = amgPd_test.Category_list.astype('category', categories=item_u).cat.codes

data = np.array([1 for k in range(len(amgPd_test))])
...
np.savetxt(pathwrite+'test_recom1_method1.csv', dfMtrx, delimiter=",",fmt='%s')
