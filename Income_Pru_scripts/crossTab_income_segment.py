# -*- coding: utf-8 -*-
"""
Created on Wed Oct 12 16:31:59 2018

@author: Latize

This script generating feasible data to be used inthe form of a matrix. Please select specic module as per the requirements
"""
# Library Declaration
import pandas as pd
import numpy as np
...
dirs = dirs[1:]
print (dirs)
# Reading Clean data (NO GI records)
amgPd = pd.read_csv(pathread+'final_data_recom_NoGIData.csv', low_memory = True)
amgPd.columns
print (amgPd.shape)
amgPd.head()
...
## Adding policy start date to generate training and testing data
...
len(amgPd_w_time)
# training data consisting of policy purchases information till 31/03/2016 by the households  # records
amgPd_w_time1 = amgPd_w_time[amgPd_w_time['PolicyStartDate'] < '2016-04-01 00:00:00.0000000']
#writing file for future use (if any)
amgPd_w_time1.to_csv(pathwrite+'training_Data_Segement_till_2016.csv', header = True, index = False)
...
# Writing files for future use (if any)
amgPd_w_time2.to_csv(pathwrite+'test_Data_Data_Segment_till_2016.csv', header = True, index = False)
...
## Training and Test data (Randomly Created 60:40)
...
#item_seg = list(sorted(amgPd.MicroSegmentProfile.unique()))
...
col1 = amgPd_w_time1.MicroSegmentProfile.astype('category', categories=item_seg).cat.codes

data = np.array([1 for k in range(len(amgPd_w_time1))])

sparse_matrix = csr_matrix((data, (row, col)), shape=(len(user_u), len(item_u)))

df_tmp1 = pd.SparseDataFrame([pd.SparseSeries(sparse_matrix[i].toarray().ravel(-1), fill_value=0) for i in np.arange(sparse_matrix.shape[0])],index=user_u, columns=item_u, default_fill_value=0)

sparse_matrix2 = csr_matrix((data, (row, col1)), shape=(len(user_u), len(item_seg)))

df_tmp2 = pd.SparseDataFrame([pd.SparseSeries(sparse_matrix2[i].toarray().ravel(-1), fill_value=0) for i in np.arange(sparse_matrix2.shape[0])],index=user_u, columns=item_seg, default_fill_value=0)
# Converting aggregate numbers of segments to binary '0' or '1'
df_tmp2 = pd.SparseDataFrame(np.where(df_tmp2 > 1, 1, df_tmp2))
...
finCols1 = ['hid']
finCols1.extend(df_tmp2.columns)
...
dfMtrx1 = np.empty(shape = (df_tmp2.shape[0]+1,df_tmp2.shape[1]+1), dtype=np.ndarray)
dfMtrx1[:1,:][0] = finCols1
dfMtrx1[1:,0] = user_u
dfMtrx1[1:,1:] = df_tmp2.values
print(dfMtrx1.shape)

dfMtrx_fnl = np.concatenate((dfMtrx,dfMtrx1), axis = 1)
print(dfMtrx_fnl.shape)

np.savetxt(pathwrite+'training_recom1_Segment_withTime2016.csv', dfMtrx_fnl, delimiter=",",fmt='%s')
