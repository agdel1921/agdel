...
pathInp_reco = "E:/NTUC/raw_data/Final/hh_result_final/FinalResults_with_Bucket/"
pathInp_nb = "E:/NTUC/raw_data/clean_raw_data/Final_recom_data/"
pathOp_merged = "E:/NTUC/raw_data/Results/Recommendation/"

reco_Pd = pd.read_csv(pathInp_reco + "pd_weighted_recommended_method11.csv", low_memory = True)
nb_Pd = pd.read_csv(pathInp_nb+"test_randomData.csv", low_memory = True)
# Reducing the size of the file
nb_Pd = nb_Pd[['hid', 'policyseqid', 'productsubcategory', 'Category_list']]
...
#                    print(prod_purchased)
    ...
    match_results_Pd.to_csv(pathOp_merged+match_method+"_accuracy_method1.csv", header=True, index=False)
...
#    newSalesPd = nb_Pd[nb_Pd["batctrcde"]!="B522"]
    ...
    print ("Total sell of test data ", str(nb_Pd.shape[0]))
    print ("Total user sales to consider of test sell data  is only", str(finalSales.shape[0]))
    finalSales2 = finalSales.fillna(0.0)
    finalSales2['hid'] = finalSales2['hid'].astype(str)
    print ("Total sell to consider of test data", str(finalSales2.shape[0]))
    #fin_uId = [int(k) for k in list(finalSales2['cownnum'])]

    # convert cownnum to int values from float
    reco_Pd = pd.read_csv(pathInp_reco + "pd_weighted_recommended_method11.csv", header = 0)
    ...
    merged_reco_nb_Pd.to_csv(pathOp_merged+ 'merged_method1.csv', header = True, index = False)
