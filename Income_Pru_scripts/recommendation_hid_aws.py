...
pathread =  "s3://ulysses-out/recom_data/"
pathwrite =  "s3://ulysses-out/recom_data/trainingData_recom1_fin.csv"
...
for ii in range(n_iteration):
    ...
    for i, Wi in enumerate(W1.T):
        temp_1__diag = diags(Wi, 0)
        print(1)
        temp_1__dot_1 = np.dot(temp_1__diag, X1)
        print(2)
        temp_1__dot_2 = np.dot(X1.T, temp_1__dot_1)
        print(3)
        temp_1__ident = np.eye(n_factor)
        print(4)
        temp_1__sum = temp_1__dot_2 + lambda1 * temp_1__ident
        print(5)
        temp_2__dot_1 = np.dot(temp_1_diag, Q1[:, i])
        print(6)
        temp_2__dot_2 = np.dot(X1.T, temp_2__dot_1)
        print(7)
        temp_3__sum = np.linalg.solve(temp_1__sum, temp_2__d)
        print(9)
        #Y1[:,i] = np.linalg.solve(np.dot(X1.T, np.dot(np.diag(Wi), X1)) + lambda1 * np.eye(n_factor),np.dot(X1.T, np.dot(np.diag(Wi), Q1[:, i])))
