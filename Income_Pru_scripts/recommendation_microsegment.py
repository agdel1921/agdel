...
data = pd.read_csv(pathwrite+'MicroSegmentProfile_recom1_fin.csv', low_memory = True)
...
def print_recommendations(W1, Q1, Q_hat3, movie_titles, type1 = "original"):
        ...
        #    Q_hat[Q_hat < 1] *= 5
            ...
            if type1!="original":
                ...
                trainedPd.to_csv(pathread+"_original_MSG_L20.csv", header=True, index=False)
            else:
                Q_hat3 *= float(1) / np.max(Q_hat3)
                tmp1 = Q_hat3 - 1 * W1
                movie_ids = np.argsort(-tmp1)[:,:7]
                trainedPd = pd.DataFrame(Q_hat3, columns = movie_titles, index = userIds)
                trainedPd.to_csv(pathread+"_weighted_MSG_L20.csv", header=True, index=False)
            ...
            if type1=="original":
                ...
                outpPd.to_csv(pathread + 'pd_recommended_MSG_L20.csv',header=True, index=False)
                #np.savetxt(pathOp_h1b1 + 'recommended'+x[-15:-11]+'.csv', og_recommend, delimiter = ",",fmt='%s')
            else:
                outpPd = pd.DataFrame(og_recommend[1:], columns = list(og_recommend[:1]))
                outpPd.to_csv(pathread + 'pd_weighted_recommended_MSG_L20.csv',header=True, index=False)
                #outpPd.to_csv(pathOp_h1b1 + 'pd_recommended'+x[-15:-11]+'.csv',header=True, index=False)
...
data.head()
userIds = np.array(data.MicroSegmentProfile)
data1 = data.drop('MicroSegmentProfile', 1)
values=data1.values
data.shape
# Create a placeholder dataframe listing item vs. item
data_ibs = pd.DataFrame(index=data.MicroSegmentProfile,columns=data1.columns, data=data1.values )
