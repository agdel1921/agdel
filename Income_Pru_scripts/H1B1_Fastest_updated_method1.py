...
data = pd.read_csv(pathwrite+'training_recom1_method1.csv', low_memory = True)
#data = pd.read_csv(pathwrite+'training_Data_withTime2016.csv', low_memory = True)
newbizPd = pd.DataFrame()

#prod_titles=data1.columns
def print_recommendations(W1, Q1, Q_hat3, prod_titles, type1 = "original"):
        ...
        #    Q_hat[Q_hat < 1] *= 5
            ...
            if type1!="original":
                ...
                trainedPd.to_csv(pathread+"_original_method1.csv", header=True, index=False)
            else:
                Q_hat3 *= float(10) / np.max(Q_hat3)
                tmp1 = (Q_hat3 - 1 * W1)
                movie_ids = np.argsort(-tmp1)[:,:7]
                trainedPd = pd.DataFrame(Q_hat3, columns = prod_titles, index = userIds)
                trainedPd.to_csv(pathread+"_weighted_method1.csv", header=True, index=False)

            for jj, prod_id_top in zip(range(m), movie_ids):
                ...
                colNum = 5
                for prod_id in prod_id_top:
                    #if Q_hat3[jj, movie_id] < 0.1: continue
                    og_recommend[jj+1][colNum]= prod_titles[prod_id]
                    og_recommend[jj+1][colNum+1]= Q_hat3[jj, prod_id]
                    colNum = colNum+2
                    print ('\n User {} recommended products is {} - with predicted rating: {}'.format(jj + 1, prod_titles[prod_id], Q_hat3[jj, prod_id]))
                print('\n' + 100 *  '-' + '\n')
            if type1=="original":
                ...
                outpPd.to_csv(pathread + 'pd_recommended_method1.csv',header=True, index=False)
                #np.savetxt(pathOp_h1b1 + 'recommended'+x[-15:-11]+'.csv', og_recommend, delimiter = ",",fmt='%s')
            else:
                outpPd = pd.DataFrame(og_recommend[1:], columns = list(og_recommend[:1]))
                outpPd.to_csv(pathread + 'pd_weighted_recommended_method1.csv',header=True, index=False)
                #outpPd.to_csv(pathOp_h1b1 + 'pd_recommended'+x[-15:-11]+'.csv',header=True, index=False)
